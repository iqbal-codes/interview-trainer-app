...
export function useInterview(options: UseInterviewOptions = {}) {
  ...
  const startInterview = useCallback(async (jobTitle: string, jobDescription?: string) => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      vapi.on('message', (message) => {
        ...
        if (message.type === 'transcript' && message.role === 'user') {
          ...
          if (session?.currentQuestionIndex !== -1 && session?.questions[session.currentQuestionIndex]) {
            updateAnswer(
  ...
  }, [initializeSession, initializeVapi, addQuestion, updateAnswer, completeSession, session, options]);
